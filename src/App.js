...
function App() {
  ...
  const randomColors = () => {
    ...
    dispatch({
      type: "SET_WRINKLES",
      payload: {
        wrinklesforeheadType: !!Math.round(Math.random() - 0.4)
          ? randomize(state.wrinkles.nTypesWrinklesForehead, "ceil")
          : "",
        wrinklesforeheadColor: palette[2],
        wrinklescheekbonesType: !!Math.round(Math.random() - 0.3)
          ? randomize(state.wrinkles.nTypesWrinklesCheekbones, "ceil")
          : "",
        wrinklescheekbonesColor: palette[2],
        wrinklesmouthType: !!Math.round(Math.random() - 0.4)
          ? randomize(state.wrinkles.nTypesWrinklesMouth, "ceil")
          : "",
        wrinklesmouthColor: palette[2],
        wrinkleschinType: !!Math.round(Math.random() - 0.4)
          ? randomize(state.wrinkles.nTypesWrinklesChin, "ceil")
          : "",
        wrinkleschinColor: palette[2],
        wrinkleseyesType: !!Math.round(Math.random() - 0.3)
          ? randomize(state.wrinkles.nTypesWrinklesEyes, "ceil")
          : "",
    ...
    dispatch({
      ...
      payload: {
        ...
        mustachioType: !!Math.round(Math.random() - 0.4)
          ? randomize(state.facialhair.nTypesMustachio, "ceil")
          : "",
        beardColor: palette[5],
        beardType: !!Math.round(Math.random() - 0.5)
          ? randomize(state.facialhair.nTypesBeard, "ceil")
          : "",
      },
    });
    ...
    dispatch({
      ...
      payload: {
        color: palette[5],
        type: !!Math.round(Math.random() - 0.2)
          ? randomize(state.glasses.nTypes, "ceil")
          : "",
      },
    });
  };
  ...
  return (
    <div className="App">
      ...
      {clickLayer && (
        <div className="click-layer" onClick={handleClickLayer}></div>
      )}
    </div>
  );
}
